...
class RitetagPy:
    def __init__(self,
        ...
        self.logfolder = get_logfolder(
            self.fb_userid, self.multi_logs, Settings)
        ...
        try:
            ...
            try:
                for cookie in pickle.load(open('{0}{1}_cookie.pkl'
                                               .format(self.logfolder, self.fb_userid), 'rb')):
            ...
            if cookie_loaded:
                self.logger.info("Issue with cookie for user {}. Creating "
                                 "new cookie...".format(self.fb_userid))

            input_fb_userid = self.browser.find_element_by_xpath(
                "//input[@id='email']")
    ...
    def update_color_in_db(self, term, col, logger):
        try:
            ...
            with conn:
                ...
                cur.execute("SELECT * FROM termColor WHERE term=:name_var", {"name_var": term})
                data = cur.fetchone()
                upload_data = dict(data) if data else None

                if upload_data is None:
                    cur.execute("INSERT INTO termColor (term, col) VALUES (?, ?)", (term, col))
                else:
                    sql = "UPDATE termColor set col = ? WHERE term = ?"
                    cur.execute(sql, (col, term))

                conn.commit()


        except Exception as exc:
            logger.error("Dap! Error occurred with update color:\n\t{}".format(str(exc).encode("utf-8")))
    ...
    def get_reports(self, term):
        ...
        for p in ps:
            ...
            if "Use this hashtag to get seen now" in p.text:
                ...
                col = 2
            elif "Do not use this hashtag, very few people are following it" in p.text:
                col = 1
        self.update_color_in_db(term, col, self.logger)

    def run(self):
        try:
            self.login_browser()
            self.get_reports("job")
            self.get_reports("kohli")
            self.get_reports("virat")
            self.get_reports("rohit")
            self.get_reports("msd")
        except Exception as e:
            print(e)
        self.browser.quit()
